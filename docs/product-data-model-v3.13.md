...
`V3.13.4：会话与分析结果入库` 已经完成，当前最近对话、助手回答快照和分析运行结果已经接入产品管理库。

我建议下一步做 `V3.13.5：Schema 同步入库`：

- 把 `/api/schema/overview` 的实时读取结果同步到 `schema_tables`、`schema_columns`、`schema_relationships`。
- 让 3D 关系图谱优先读取产品库里的 schema 快照，而不是每次直接扫业务库。
- 在系统设置的数据源页面增加“同步 Schema”动作和最近同步时间。
- 为后续 RAG schema 检索、权限控制和多数据源切换打基础。

这样数据结构模块会从“实时展示工具”升级为“可管理、可审计、可复用的元数据资产”。
