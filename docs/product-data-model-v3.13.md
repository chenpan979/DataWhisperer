...
`V3.13.1：数据库迁移脚本` 已经完成，当前已经有了可以反复执行的产品管理库初始化脚本。

我建议下一步做 `V3.13.2：后端 Repository 层`：

- 为租户、用户、工作空间、数据源、会话和分析结果补充 SQLAlchemy models。
- 封装 repositories，把数据库读写从 API 和业务编排代码里拆出去。
- 先打通账号、工作空间、数据源和会话这条主链路，再逐步把 RAG 与评测数据入库。

这样前端现在看到的“登录、系统设置、最近对话、数据源管理”，后面都能自然接到真实数据库，而不是继续依赖浏览器 localStorage 或本地 JSON 文件。
