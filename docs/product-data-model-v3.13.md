...
`V3.13.3：登录页接后端` 已经完成，当前账号体系已经从前端演示状态接到了产品管理库。

我建议下一步做 `V3.13.4：会话与分析结果入库`：

- 把当前 `storage/chat_conversations.json` 的会话存储迁移到 `conversations` 和 `chat_messages`。
- 把每次 AI 查数的 SQL、图表、表格预览、结论和追问建议保存到 `analysis_runs`。
- 最近对话列表按当前用户和工作空间从产品库读取。
- 保留旧 JSON 文件作为一次性迁移来源或本地兜底，避免历史演示数据突然丢失。

这样登录后的每个用户都会看到自己的真实会话历史，产品会更接近多租户 SaaS 的状态。
