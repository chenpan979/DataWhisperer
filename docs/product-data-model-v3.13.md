...
- 当前版本已完成这一项，产品管理库命名为 `datawhisperer_product`，示例业务库仍然保留为 `datawhisperer_demo`。
...
- 当前版本已完成基础产品库 ORM 和 Repository 层，覆盖租户、用户、工作空间、数据源、会话、分析结果和审计日志的核心读写。
...
- 当前版本已完成登录、注册、找回密码占位和当前用户查询接口，前端登录页已经改为请求真实后端。

### V3.13.4：会话与分析结果入库

- 把当前 `storage/chat_conversations.json` 迁移到数据库。
- `conversations`、`chat_messages`、`analysis_runs` 真正落库。
- 当前版本已完成这一项，会话按租户、工作空间和用户隔离；旧 JSON 会话会在 demo 管理员首次读取最近对话时尝试迁移。

### V3.13.5：Schema 同步入库

- `/api/schema/overview` 登录后优先读取 `schema_tables`、`schema_columns`、`schema_relationships` 快照。
- 新增 `POST /api/schema/sync`，把默认数据源的表、字段和外键关系同步到产品管理库。
- 新增 `GET /api/schema/tables` 和 `GET /api/schema/tables/{table_id}`，为数据结构页面、表详情和后续 schema 检索提供统一读取入口。
- 3D 图谱刷新时会触发 schema 重新同步，图谱渲染优先使用产品库快照。
- 当前版本已完成这一项，schema 不再只是一次性实时展示，而是变成可管理、可审计、可复用的元数据资产。

### V3.13.5.1：会话恢复稳定性修复

- 修复 AI 查数回答快照保存时序，避免结果刚展示时刷新导致保存请求被浏览器中断。
...
### V3.13.5.4：历史对话一比一恢复修复

- 历史恢复不再重新拼一套“历史样式”卡片，而是优先复用 `assistantHtml` 保存的原始回答卡片。
- 图表区域会用 `chart` 结构化配置重新挂载 ECharts，避免图片快照截早、空轴或样式不一致。
- 会话内上一轮回答归档时保留详细数据、生成 SQL 和追问建议入口，不再隐藏整条结果详情操作区。
- `static/index.html` 和 `bootstrap.js` 静态资源版本升级到 `3.13.5.4`。

### V3.13.6：数据源管理接后端

- 系统设置里的数据源配置从前端草稿升级为真实后端 API。
- 支持创建、修改、测试连接、设置默认数据源和查看最近同步时间。
- 数据源密码后续需要接入服务端加密或密钥管理，本阶段可以继续使用本地演示加密占位。
...
`V3.13.5：Schema 同步入库` 已经完成，当前默认数据源的表、字段和外键关系已经可以同步进产品管理库。

我建议下一步做 `V3.13.6：数据源管理接后端`：

- 把系统设置中的数据源表单接入真实 API，而不是只保存在前端本地草稿。
- 支持数据源列表、默认数据源、连接测试和 Schema 同步状态展示。
- 让后续多租户、多数据源切换有真实的产品入口。
- 为后续把 AI 查数从固定示例库升级为“按工作空间选择数据源”打基础。

这样系统设置会从“静态配置页”升级为“真实的数据源管理中心”。
