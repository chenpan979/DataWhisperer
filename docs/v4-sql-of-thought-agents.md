# V4.1.2 SQL-of-Thought 多智能体执行可视化设计
...
为了让后续多智能体扩展更清楚，V4.1.1 将原来集中在 `sql_of_thought.py` 里的 Agent 拆成独立模块：

- `schema_linking.py`：负责读取并压缩数据库 schema。
- `knowledge_retrieval.py`：负责内置指标口径和工作空间知识库检索。
- `query_planning.py`：负责生成 SQL-of-Thought 查询计划。
- `sql_generation.py`：负责调用 SQL Agent 生成 SQL。
- `validation_execution.py`：负责 SQL 安全校验、执行和一次自动修复。
- `charting.py`：负责基于结果推荐图表。
- `insight.py`：负责生成业务分析结论。
- `orchestrator.py`：只负责 Agent 顺序编排和共享状态流转。

`app/agents` 是当前真实运行目录；`app/agent` 是 V1-V3 遗留兼容入口，不再放新的 Agent 实现。`sql_of_thought.py` 也保留为兼容导出模块，避免旧脚本或历史测试导入失败。

## V4.1.2 对话页执行进度

多智能体拆分以后，用户需要在等待过程中看见系统不是“黑盒思考”。V4.1.2 在 AI 查数结果卡片顶部新增 Agent 进度下拉：

- 收起时显示当前正在执行的 Agent 和完成数量。
- 展开时展示 SchemaLinkingAgent、KnowledgeRetrievalAgent、QueryPlanningAgent、SQLGenerationAgent、ValidationExecutionAgent、ChartAgent、InsightAgent 的完整链路。
- 等待阶段使用前端进度动画表达执行感，真实响应返回后再用后端 trace_steps 校准每个 Agent 的最终状态。
- 历史对话仍保留进度卡片，方便复盘某次查询经过哪些 Agent。
...
- V4.1.1：已完成 Agent 文件拆分，一个 Agent 一个模块，`app.agent` 只作为旧导入兼容层。
- V4.1.2：已完成对话页 Agent 执行进度可视化，等待和完成状态都能看到多智能体链路。
