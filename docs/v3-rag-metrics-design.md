...
### V3.5：数据结构与 RAG 知识库管理台

V3.5 将前端从单一 AI 查数页面升级为三个工作区：

- AI 查数：保留自然语言分析、图表、SQL 和执行轨迹。
- 数据结构：管理表结构、字段说明、建表 SQL、数据字典等资料文件。
- RAG 知识库：管理业务口径、FAQ、分析规则、SQL 样例等知识文件。

当前版本先实现资料上传、列表、文本预览和删除，原因是：

- 先把“资料运营入口”做出来，项目更像真实产品。
- 数据结构资料和 RAG 资料分开管理，后续解析和索引边界更清晰。
- 上传文件先存入 `storage/`，不会污染 `knowledge/metrics/` 这类已版本化指标口径。
- 后续可以在此基础上继续扩展 CSV 入库、schema 自动解析、RAG 切片和 Milvus 索引同步。

### V3.6：分析体验增强控制台

V3.6 重点优化 AI 查数页的等待体验和结果解释体验：

- 将执行步骤升级为可展开时间线，展示理解问题、读取 schema、检索指标、生成 SQL、执行查询、生成图表和生成结论的完整过程。
- 分析结论使用前端逐字输出，降低用户等待时的割裂感。
- 图表加入 ECharts tooltip、缩放、还原、保存图片和点击反馈。
- 结果下方自动生成 3-5 个追问建议，引导用户继续分析。
- 统一按钮、标签页、面板和时间线的微动效，保持控制台体验克制、连贯。
...
- 将 V3.5 上传的 RAG 知识库文件接入自动切片和 Milvus 同步。
- 将 V3.5 上传的数据结构文件接入 schema 解析和表字段补充。
- 增加指标冲突检测。
- 增加“未命中指标”的用户提示。
- 将指标检索工具包装成 MCP 工具。
