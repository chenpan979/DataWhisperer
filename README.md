...
当前项目已经更新到 **V4.1.2：多智能体执行进度可视化版本**。
...
- `v3.11.3`：补全最近对话管理能力，支持会话内联重命名和删除，删除当前会话后自动切换到可用会话。
- `v3.11.4`：新增会话服务端持久化能力，AI 查数会话、重命名和历史问答会保存到本地 `storage/chat_conversations.json`，刷新页面或重启后端后仍可恢复。
- `v3.11.5`：将会话历史从摘要持久化升级为完整结果快照持久化，恢复历史会话时可以看到结论、图表预览、详细数据、生成 SQL、告警和追问建议。
- `v3.11.6`：优化历史回答恢复形态，优先用 ECharts 还原原始图表样式，并隐藏本地演示 SQL 提示等非核心告警，让重启后的会话更接近原回答。
- `v3.11.7`：修复浏览器缓存和 ECharts CDN 未加载时的历史图表恢复问题，历史回答会优先原样渲染，兜底图表也保持浅色商务风格。
- `v3.11.8`：将 AI 查数历史恢复升级为回答卡片 HTML 快照恢复，重启后尽量一比一还原图表、详细数据下拉、生成 SQL 下拉、追问小灯泡和按钮间距。
- `v3.11.9`：将系统设置拆分为数据源、模型、账号和安全四个页签，账号偏好新增头像上传、岗位角色和修改密码草稿校验，并同步左下角用户卡片。
- `v3.11.10`：修复系统设置页签面板被卡片样式覆盖导致全部展开的问题，并将页签入口上移到设置页面顶部，交互更接近数据结构模块。
- `v3.11.11`：优化系统设置页签定位，切换账号偏好等长内容时页签保持在设置区顶部，避免页面跳到表单正文后看不到分类导航。
- `v3.11.12`：继续修正系统设置布局，将概览卡片收进数据源配置页签，账号偏好切换后只展示账号页内容，页签样式更贴近数据结构模块。
- `v3.11.13`：重构系统设置为标题区、页签区、内容区三段式布局，确保数据源配置、模型配置、账号偏好和安全策略像数据结构模块一样稳定切换。
- `v3.11.14`：优化账号偏好页，将个人资料和密码安全拆成左右双栏，避免表单输入框在大屏下横向铺满导致信息密度过低。
- `v3.11.15`：拆分前端入口，将 SVG 图标库和页面骨架从 `index.html` 拆到 `static/partials`，由 `bootstrap.js` 装配后再启动业务脚本，降低后续维护成本。
- `v3.12.0`：新增登录、注册和忘记密码前端入口，围绕工作空间/租户标识设计账号流程，为后续多租户后端鉴权、组织隔离和权限控制预留字段。
- `v3.12.1`：优化登录页视觉层级，去掉粗重分段 Tab，将登录作为主路径，把注册和找回密码收敛为轻量入口，整体更贴近企业级云产品登录体验。
- `v3.12.2`：继续产品化登录页，移除顶部模式切换入口，精简开发阶段文案，并关闭浏览器原生表单气泡，改为页面内的轻量提示。
- `v3.13.0`：新增产品数据模型设计文档，规划多租户、用户、工作空间、数据源、Schema、AI 查数、RAG、评测和审计的 E-R 图与落地顺序。
- `v3.13.1`：新增产品管理库初始化脚本 `scripts/init_product_schema.sql`，落地租户、用户、工作空间、数据源、Schema、会话、分析结果和审计等核心表。
...
- `v3.13.5.1`：修复 AI 查数结果保存时序，等待完整回答快照入库后再显示完成，并支持退出后重新登录恢复完整对话。
...
- `v4.1.2`：升级 AI 查数等待态，新增多智能体执行进度下拉卡片，实时展示当前执行到哪个 Agent，完成后仍可展开查看 Agent 链路。

项目第一阶段重点不是堆概念，而是先做出一个能真实跑通的 Text-to-SQL 数据分析闭环。V2 补充大模型工程化能力，V3 加入 RAG 业务知识增强，V4 开始把查数链路升级为 SQL-of-Thought 多智能体架构，V4.1 把系统设置中的 Agent 模型绑定接入运行时，V4.1.1 进一步把多智能体代码拆成一 Agent 一文件，V4.1.2 把多智能体执行过程显性化展示到对话界面，后续会继续扩展多库路由、MCP 工具化和 Agent 协作评测。
...
- V4.1.2 在对话结果顶部新增 Agent 进度下拉，用户可以看到当前执行到 SchemaLinkingAgent、SQLGenerationAgent、InsightAgent 等哪一步。
...
- V3.12.0 新增登录、注册、忘记密码前端页面，并在账号流程里预留工作空间/租户标识，为后续多租户隔离、成员管理和权限体系打基础。
- V3.12.1 重做登录卡片的模式切换和表单视觉，登录页不再使用笨重的三段式 Tab，注册/找回密码改为轻量辅助入口。
- V3.12.2 移除登录卡片顶部的账号登录/创建空间/找回密码入口，精简 UI 文案和错误提示，让登录页更接近真实产品入口。
- V3.13.0 开始补齐产品级底座设计，新增 [产品数据模型设计文档](docs/product-data-model-v3.13.md)，明确 E-R 图、核心表结构、多租户隔离规则和后续数据库迁移顺序。
- V3.13.1 新增 [产品管理库初始化脚本](scripts/init_product_schema.sql)，使用非破坏式 SQL 初始化 `datawhisperer_product`，并写入 demo 租户、管理员、默认工作空间和示例 MySQL 数据源。
...
- V3.13.5.1 修复前端保存时序：AI 查数结果渲染后会等待回答快照写入 `/api/chat/conversations/{id}/turns`，重新登录时也会重新拉取最近对话，避免刷新或退出后历史问题丢失。
...
- V3.13.11 补齐 RAG 文件上传到向量库的闭环：上传后自动切片、生成 embedding、写入独立的 Milvus collection，并把同步结果回写到文件元数据。
- V3.13.12 将知识库正式纳入多租户产品库：每个工作空间自动拥有默认知识库，上传文档会登记到 `knowledge_documents`，切片会记录到 `knowledge_chunks`，并把 Milvus 写入元数据带上租户和工作空间边界。
- V3.13.13 补齐知识库检索侧隔离：AI 查数前会基于当前工作空间默认知识库检索上传资料，检索结果注入 SQL prompt，并在响应中返回 `retrieved_knowledge`。
...
flowchart TD
    ...
    API --> MR["AgentModelRouter: 工作空间模型绑定"]
...
批量更新 Agent 模型绑定。V4.1 已经把 SQL 生成、SQL 修复和分析总结接入运行时路由，后续还可以继续扩展到 RAG、图表和多库路由 Agent。
...
当前文件上传已经支持资料管理、控制台预览，以及 RAG 知识库文件自动切片同步 Milvus。数据结构文件解析和 CSV 入库仍可作为后续增强方向。
...
- V4.1.2：对话页展示多智能体执行进度，让用户能感知当前执行到哪个 Agent。
